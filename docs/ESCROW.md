...
| **Durata dei lock (R1)** | Un singolo HTLC di settimane = liquidità congelata e rischio force-close                                                                                | **Rinnovo rolling del bond hub** ([ADR-033](adr/ADR-033-rolling-bond-renewal.md)): ogni hold copre ≤7 giorni (budget CLTV sano, ESCROW storico §4) e il coordinatore la rimpiazza automaticamente prima della scadenza — la giacenza arriva così a **30 giorni** (ADR-026 Parte 2). Mancato rinnovo = fine giacenza anticipata (TOS §10). Limite noto: la hold `Π_h` (ADR-014) e le hold di un claim non rinnovano ancora — oltre la loro finestra scadono e degradano senza perdita (il premio torna al mittente) |
