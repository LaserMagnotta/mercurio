...
| **Durata dei lock (R1)** | Il lock più lungo è il bond hub = intera giacenza. HTLC di settimane = liquidità congelata e rischio force-close                                                                                | **Giacenza massima 7 giorni nell'MVP** (budget CLTV sano; l'unità è il giorno — [ADR-026](adr/ADR-026-storage-in-days.md)); il **rinnovo rolling del bond** è la via decisa per arrivare a 30 giorni (ADR-026 §3, opzione A), da implementare a parte |
