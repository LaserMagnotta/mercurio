...
export const hubs = pgTable(
  ...
  // Viewport (bbox) discovery queries filter on lat AND lng (ADR-030): a
  // btree on the pair keeps them index-assisted at the 10k-hub target scale
  // without dragging in PostGIS for an MVP.
  (table) => [index('hubs_lat_lng_idx').on(table.lat, table.lng)],
);
